...
const aliases: Record<string, ConfigAlias> = {
  ...
  "github.public-read-token": { secret: true, patch: value => ({ github: { publicReadToken: value } }) },
...
function printSnapshot(store: SettingsStore, json = false): void {
  ...
  stdout.write([
    ...
    `Public read token  ${snapshot.github.publicReadTokenConfigured ? "configured" : "optional"}`,

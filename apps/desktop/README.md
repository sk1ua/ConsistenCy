...
Use Node.js 22.19+ (22.x only) and Python 3.12:
...
The Node helper is checked at startup and must report Node 22.19 or newer within the 22.x line. No Electron ABI
...
The packer refuses Node versions older than 22.19 or outside Node 22, bundles the API, copies the active
...
publish metadata generated by electron-builder. Pi's API runtime is bundled,
but the packer does not copy the current user's Pi `auth.json`, `models.json`,
or catalog cache into staging or the installation directory; the API helper
reads Pi configuration only at runtime through its server-side environment
allowlist.

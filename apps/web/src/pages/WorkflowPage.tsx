 ...
 * repository, and persisted run history. The legacy builder remains frozen; the
 * runtime-native graph Studio is isolated in RuntimeStudio and is not a legacy redesign.
 *
...
export function WorkflowPage({
  ...
  // when no ?tab= is present. Every legacy ?tab= value (triggers / runtime /
  // studio / xray / definition) remains a valid deep link — tab ids unchanged.
  const activeTab =
    searchParams.get("tab") === "triggers" ? "triggers"
      : searchParams.get("tab") === "runtime" ? "runtime"
        : searchParams.get("tab") === "definition" ? "definition"
          : searchParams.get("tab") === "xray" ? "xray"
            : searchParams.get("tab") === "studio" ? "studio"
              : "studio";
  ...
  return <div className="page-stack workflows-page">
    <div className="workflow-sub-nav" role="tablist" aria-label={t("Workflow views")}>
      ...
      <button
        ...
        ref={element => registerWorkflowTab("definition", element)}
        onClick={() => setSearchParams({ tab: "definition" })}
      >
        <Workflow size={14} />
        {zh ? "工作流定义" : "Definitions"}
        <span className="ds-badge ds-badge--sm ds-badge--neutral">{t("Legacy · read-only")}</span>
      </button>
    </div>

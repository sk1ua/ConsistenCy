...
export function ModelSettingsSection({
  ...
  return <section className="settings-group section-block">
    ...
    <div className="settings-fields">
      <div className="setting-field"><label htmlFor="setting-provider">{t("Provider")}</label><select id="setting-provider" aria-describedby="setting-provider-help" value={draft.llm.provider ?? "none"} onChange={event => updateLlm({ provider: event.target.value as SettingsSnapshot["llm"]["provider"] })}><option value="none">{t("Not configured")}</option><option value="deepseek">DeepSeek</option><option value="openai">OpenAI</option><option value="pi">Pi</option></select><SettingHelp id="setting-provider-help" text="ConsistenCy requires a real LLM Provider (DeepSeek, OpenAI, or Pi) to execute reviews." /></div>
      ...
      {draft.llm.provider === "pi" && <>
        <div className="setting-field setting-field-wide"><label htmlFor="setting-pi-model">{t("Pi model")}</label><input id="setting-pi-model" aria-describedby="setting-pi-model-help" value={draft.llm.piModel ?? ""} onChange={event => updateLlm({ piModel: event.target.value })} placeholder="provider/model" /><SettingHelp id="setting-pi-model-help" text="Uses the model catalog and credentials from Pi's server-side agent configuration. Enter only provider/model; credentials never enter ConsistenCy settings." /></div>
        <div className="setting-field setting-note"><Sparkles size={17} /><div><strong>{t("Pi configuration")}</strong><p>{t("Pi models.json and auth.json are read by the API process; no Pi credential is sent to the browser.")}</p></div></div>
      </>}
    </div>
  </section>;
}

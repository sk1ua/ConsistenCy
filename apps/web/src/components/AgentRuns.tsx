...
export function AgentRuns({ runs }: { runs: AgentRun[] }) {
  return <div className="agent-timeline">{runs.map(run => {
    ...
    return <div className={`agent-run agent-${run.status}`} key={run.id}>
      ...
      <div className="agent-meta"><strong>{run.findings.length}</strong><span>findings</span></div>
      <div className="agent-meta"><strong>{duration(run)}</strong><span>{run.status}</span></div>
